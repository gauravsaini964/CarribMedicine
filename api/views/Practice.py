...
class QuizInitialize(APIView):
    ...
    def post(request):
        ...
        questions = Questions.objects.filter(
            id__in=questions_list, flag=True, question_type='mcq', question_text__isnull=False).order_by('?')[:10]
        final_question_list = questions.values_list('id', flat=True)
        
        quiz_obj = Quiz.objects.create(
            user_id=user, quiz_type='practice', result_set={}, entity_id=subject['id'], entity_name=subject['name'])
        final_questions = []
        for question_id in final_question_list:
            quiz_questions = QuizQuestions.object.create(quiz_id=quiz_obj.id, question_id=question_id)
            ...
            question_choices = QuestionsChoices.objects.filter(
                question_id=question_id).values(
                    'choice__text', 'choices__media_url', 'choice__media_type', 'choice__choice_type', 'choice__hints',
                    'choice_id')
            temp_dict = {'question': question_obj, 'choices': question_choices, 'quiz_question_id': quiz_questions.id}
            final_question_list.append(temp_dict)
